...
int main() {
	...
	while (inputting) {
		...
		if (strcmp(command, "ADD") == 0) {
			...
			cout << "\nEnter \'ADD\', \'PRINT\', \'DELETE\' to continue or \'QUIT\' to exit." << endl << endl;
		} 
...
void print_students(Student** hash_table, int num_slots) {
	...
	for (int i = 0; i < num_slots; i++) { //iterate through hash table
		...
		while (current != NULL) { //print out student(s) at hash index
			...
			cout << "\nFirst: " << current->first_name << endl;
			cout << "Last: " << current->last_name << endl;
			cout << "ID: " << current->id << endl;
			cout << "GPA: " << fixed << setprecision(2) << current->gpa << endl;
